    ...
    if request.method == "POST":
        try:
            ...
            ChatMessage.objects.create(
                session=chat_session, role="user", content=user_message
            )
            ...
            try:
                ...
                return JsonResponse(
                    {"error": "Invalid response from AI API"}, status=500
                )
            ...
            if response.status_code == 200 and "choices" in response_data:
                ...
                ChatMessage.objects.create(
                    session=chat_session, role="assistant", content=bot_reply
                )
...
def clear_conversation(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse(
            {"status": "Conversation cleared", "new_session_id": new_session.id}
        )
    return JsonResponse({"error": "Invalid request method"}, status=400)
                ...
                history.append(
                    {
                        ...
                        "created_at": chat_session.created_at.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
...
def export_chat_history(request, session_id):
    ...
    response["Content-Disposition"] = (
        f"attachment; filename=chat_session_{session_id}.txt"
    )
    return response
